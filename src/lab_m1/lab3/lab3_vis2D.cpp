...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_A)) logicSpace.x -= 1 * deltaTime;
    if (window->KeyHold(GLFW_KEY_D)) logicSpace.x += 1 * deltaTime;
    if (window->KeyHold(GLFW_KEY_W)) logicSpace.y += 1 * deltaTime;
    if (window->KeyHold(GLFW_KEY_S)) logicSpace.y -= 1 * deltaTime;
