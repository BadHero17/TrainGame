...
void Lab5::Update(float deltaTimeSeconds)
{
    if (isPerspective) {
        ...
        projectionMatrix = glm::ortho(camera->position.x - projectionX, camera->position.x + projectionX, camera->position.y - projectionY, camera->position.y + projectionY, 0.01f, 200.0f);
    }
...

    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1, 3, 4));
