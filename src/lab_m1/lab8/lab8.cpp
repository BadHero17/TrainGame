...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int is_directional_location = glGetUniformLocation(shader->program, "is_directional");
    glUniform1i(is_directional_location, is_directional);

    int theta_location = glGetUniformLocation(shader->program, "theta");
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) light2Position -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_LEFT)) light2Position -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_DOWN)) light2Position += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_RIGHT)) light2Position += right * deltaTime * speed;

        if (window->KeyHold(GLFW_KEY_I)) {
            glm::vec4 temp = transform3D::RotateOX(deltaTime * speed) * glm::vec4(lightDirection, 1);
            lightDirection = glm::vec3(temp.x, temp.y, temp.z);
        }
        if (window->KeyHold(GLFW_KEY_J)) {
            glm::vec4 temp = transform3D::RotateOZ(deltaTime * speed) * glm::vec4(lightDirection, 1);
            lightDirection = glm::vec3(temp.x, temp.y, temp.z);
        }
        ...
        if (window->KeyHold(GLFW_KEY_U)) theta += deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_O)) theta -= deltaTime * speed;
    }
}
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F) {
        is_directional = 1 - is_directional;
    }
