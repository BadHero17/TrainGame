...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::vec3 camera_pos = GetSceneCamera()->m_transform->GetWorldPosition();
        glm::vec3 to_grass = glm::vec3(0.0f, 0.5f, 0.0f) - camera_pos;
        float theta = glm::dot(glm::normalize(glm::vec3(to_grass.x, 0, to_grass.z)), glm::vec3(0, 0, 1));

        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0f, 0.5f, 0.0f));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
        if (glm::acos(theta) > 0) modelMatrix = glm::rotate(modelMatrix, glm::acos(theta), glm::vec3(0, 1, 0));
        else modelMatrix = glm::rotate(modelMatrix, (float)AI_MATH_PI + glm::acos(theta), glm::vec3(0, 1, 0));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    float elapsed_time = Engine::GetElapsedTime();
    int loc_elapsed_time = glGetUniformLocation(shader->program, "elapsed_time");
    if (mesh == meshes["sphere"]) {
        glUniform1f(loc_elapsed_time, elapsed_time);
    }
    else {
        glUniform1f(loc_elapsed_time, 0);
    }
